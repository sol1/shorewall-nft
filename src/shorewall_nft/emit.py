...
One dual-stack table: inet shorewall. Zone dispatch uses verdict maps.
Zone-pair chains keep upstream's naming so the ruleset stays readable
to Shorewall users.
"""
import re
...
class Emitter:
    ...
    def render(self):
        cfg = self.cfg
        # Pre-scan for +ipset references so declarations come first.
        for rule in list(cfg.rules) + list(cfg.blrules):
            _rule_match(rule, cfg.family, self.sets)
        self.out("#!/usr/sbin/nft -f")
        self.out(f"# Generated by shorewall-nft from {cfg.confdir}")
        self.out("# Do not edit. Recompile instead.")
        self.out("")
        self.out("destroy table inet shorewall")
        self.out("table inet shorewall {")
        ...
        for name in sorted(self.sets):
            ...
            self.out(f"type {addr_type}; flags interval; auto-merge;", 2)
            if defn and defn.elements:
                self.out("elements = {", 2)
                elems = defn.elements
    ...
    def _nat(self):
        ...
        for s in self.cfg.snat:
            ...
            if s.daddr:
                m.append(f"{ipkw} daddr {_addr_set(s.daddr)}")
            if s.source:
                m.append(f"{ipkw} saddr {_addr_set(s.source)}")
            if s.origdest:
