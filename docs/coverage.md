...
| actions | User-defined actions declared in the actions file and defined in action.<name>, expanded like macros. Standard actions are native: AllowICMPs accepts the needed ICMP types (RFC 4890 on IPv6, destination-unreachable and time-exceeded on IPv4); the conntrack-state actions (New, Established, Related, Invalid, Untracked and the allow/drop wrappers) match ct state; Broadcast, Multicast and their wrappers match fib daddr type; the TCP-flag actions (TCPFlags, RST, FIN, NotSyn, dropNotSyn, rejNotSyn) match tcp flags, and A_REJECT audits then rejects; DropSmurfs, DropDNSrep and GlusterFS are native too. The actions that need features not built yet (Limit, BLACKLIST, UPnP, events) fail with a located error; see docs/design/actions.md. |
