...
| actions | User-defined actions declared in the actions file and defined in action.<name>, expanded like macros. Standard actions are native: AllowICMPs accepts the needed ICMP types (RFC 4890 on IPv6, destination-unreachable and time-exceeded on IPv4); the conntrack-state actions (New, Established, Related, Invalid, Untracked and the allow/drop wrappers) match ct state; Broadcast, Multicast and their wrappers match fib daddr type. The rest of actions.std is being added natively; see docs/design/actions.md. |
