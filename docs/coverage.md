...
| netmap | Traditional 8-column IPv4/IPv6 NETMAP format, including exclusions, NET3, protocol/ports, logical/wildcard interfaces and normal chain designators, emitted as stateful nft prefix NAT. Cross-hook `SNAT:P` and `DNAT:T` fail explicitly. See [netmap.md](netmap.md). |
...
| actions | User-defined actions declared in the actions file and defined in action.<name>, expanded like macros. Standard actions are native: AllowICMPs accepts the needed ICMP types (RFC 4890 on IPv6, destination-unreachable and time-exceeded on IPv4); the conntrack-state actions (New, Established, Related, Invalid, Untracked and the allow/drop wrappers) match ct state; Broadcast, Multicast and their wrappers match fib daddr type; the TCP-flag actions (TCPFlags, RST, FIN, NotSyn, dropNotSyn, rejNotSyn) match tcp flags, and A_REJECT audits then rejects; DropSmurfs, DropDNSrep and GlusterFS are native too; AutoBL auto-blacklists a source that exceeds a rate, using a dynamic timed set and a rate meter; KNOCK and KNOCKSEQUENCE use dynamic timed state sets and an early prerouting chain, with optional NFLOG; SetEvent, ResetEvent and IfEvent (also SETEVENT/RESETEVENT/IFEVENT) implement Shorewall's Events interface with dynamic timed sets and, for a rate test, a meter, usable directly in rules or inside an action body. The actions that need features not built yet (Limit, BLACKLIST, UPnP) fail with a located error; see docs/design/actions.md, docs/knocking.md and docs/events.md. |
